...
//        return $transfers->transfers($openid,$amount,$bill_no,$desc);
        ...
        $openid = (new Visitors())->getOpenidByToken($token);
        //var_dump($openid);
        if(empty($openid)){
            $this->error(110,'openid获取失败');
        }
        //数据组装
        //$openid ='of5Ye0ac5U3qomirNfJIMm7AhR-Y';// $this->request()->getRequestParam('openid');
        ...
        //业务逻辑处理
        $content=$redpack->mchpay($openid['openid'],$money,$trade_no,$act_name);			//企业付款
        //var_dump($content);
        //var_dump($content['return_code']);
        if($content['return_code'] == 'SUCCESS'){
            //获得手机号码
            $res=Capsule::table('redpack')->where('openid','=', $openid['openid'])->get()->toArray();
            //var_dump($res);
            $res = array_map('get_object_vars', $res);
            if(!empty($res[0])){
                $rs=$res[0];
                $rs=Capsule::table('redpack')
                    ->where('phone','=', $rs['phone'])
                    ->update(
                        [
                            'money'=>$money,
...
// 调用正确，`$bill` 为 csv 格式的内容，保存为文件：
    ...
    public function userRedStatus(){
        $visitId='sd02e7f0YUhs';//$this->request()->getRequestParam('visit_id');
        if(empty($visitId)){
            $this->error(411,'参数错误');
        }
        $openid = (new VisitorWechat())->getOpenidByVisitId($visitId);
        if(empty($openid)){
            $this->error(412,'openid获取失败');
        }
        $res=Capsule::table('redpack')->where('openid','=', $openid)->get();
        if(empty($res['phone'])){
            $this->success();
        }else{
            $this->error(400,'已给予过红包');
        }
